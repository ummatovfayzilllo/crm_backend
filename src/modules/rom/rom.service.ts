...
export class RomService {
  ...
  async getLidsStats() {
    try {
      ...
      const stats = rooms.map((room) => {
        ...
        const studentCount = room.Group.reduce(
          (acc, g) => acc + g.students.length,
          0,
        );
        const lessonCount = room.Group.reduce(
          (acc, g) => acc + g.Lesson.length,
          0,
        );
  ...
  async findOne(id: string) {
    const rom = await this.prisma.rom.findFirst({ where: { id: id } });
    if (!rom) {
  ...
  async update(id: string, data: UpdateRomDto) {
    ...
    if (data.romNumber) {
      const OLdROm = await this.prisma.rom.findFirst({
        where: { romNumber: data.romNumber },
      });
      if (OLdROm && OLdROm.id !== id)
        throw new BadGatewayException(
          `Already exists rom by romNumber [${data.romNumber}]`,
        );
    }
    if (data.name) {
      const OLdROm = await this.prisma.rom.findFirst({
      ...
      if (OLdROm && OLdROm.id !== id)
        throw new BadGatewayException(
          `Already exists rom by name [${data.name}]`,
        );
    }
  ...
  async remove(id: string) {
    ...
    if (await this.prisma.group.findFirst({ where: { romId: id } })) {
      throw new BadRequestException(
        `Reference exists by romId [${id}] in group `,
      );
    }
